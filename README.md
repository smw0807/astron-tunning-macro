...
`실행` 탭 → `🔍 인스턴스 조회` → `bluestacks.conf` 의 모든 인스턴스를 이름·포트·온라인 여부와
함께 목록으로 표시. 실행할 것을 체크 → `선택 적용` → 인스턴스별 서브탭(상태·로그·시작/정지) 생성
→ `전체 시작` 으로 **병렬 개조**. 좌표/시퀀스/목표는 공유(같은 게임·해상도), ADB 대상만 다름.
포트는 매번 조회 시 conf 에서 자동 해석하므로 수동 입력 불필요. `config.yaml` › `instances:` 에 저장.
